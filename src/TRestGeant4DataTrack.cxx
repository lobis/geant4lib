...
TRestGeant4DataTrack::operator TRestGeant4Track() const {
    ...
    // Add hits
    for (int i = 0; i < fHits.GetNumberOfSteps(); i++) {
        track.AddG4Hit(fHits.GetPosition(i), fHits.GetEnergy(i), fHits.GetTime(i), fHits.GetProcessID(i),
                       fHits.GetVolumeID(i), fHits.GetKineticEnergy(i), fHits.GetDirection(i));
    }
